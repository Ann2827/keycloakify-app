...
const LoginConfigTotp = ({
  kcContext,
  theme,
  ...props
}: { kcContext: KcContextBase.LoginConfigTotp } & KcProps & { theme: ThemeTypes }) => {
      ...
      <form action={url.loginAction} className={cx(props.kcFormClass)} id='kc-totp-settings-form' method='post'>
        ...
        <div className={cx(props.kcFormGroupClass)}>
          <div className={cx(props.kcInputWrapperClass)}>
            ...
            {totp.otpCredentials.length > 0 && <span className='required'>*</span>}
          </div>
          <div className={cx(props.kcInputWrapperClass)}>
            <input
              type='text'
...
export default React.memo(LoginConfigTotp);
